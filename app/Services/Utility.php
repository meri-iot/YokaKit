<?php

declare(strict_types=1);

namespace App\Services;

use DateTimeZone;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\ModelNotFoundException;
use Illuminate\Support\Carbon;
use InvalidArgumentException;

/**
 * 汎用処理をまとめたクラス
 */
class Utility
{
    /**
     * 現在時刻を取得する
     *
     * @return Carbon
     */
    public static function now(): Carbon
    {
        // DB保存時の桁ブレを避けるため、マイクロ秒はミリ秒精度へ丸める。
        $now = Carbon::now(new DateTimeZone(config('app.timezone')));
        $millisecond = $now->millisecond;
        return $now->microseconds(0)->milliseconds($millisecond);
    }

    /**
     * 時刻を文字列に変換する
     *
     * @param Carbon $date 時刻
     * @param string $format フォーマット
     * @return string
     */
    public static function format(Carbon $date, string $format = 'Y-m-d H:i:s.u'): string
    {
        return $date->format($format);
    }

    /**
     * 年月日と時分秒のデータを結合する
     *
     * @param Carbon $ymd 年月日
     * @param Carbon $hms 時分秒
     * @return Carbon
     */
    public static function merge(Carbon $ymd, Carbon $hms): Carbon
    {
        return $ymd->copy()
            ->hours($hms->hour)
            ->minutes($hms->minute)
            ->seconds($hms->second);
    }

    /**
     * 指定した文字列を時刻へ変換する
     *
     * @param string|null $datetime 時刻文字列
     * @param string $format フォーマット
     * @return Carbon
     * @throws InvalidArgumentException
     */
    public static function parse(?string $datetime, string $format = 'Y-m-d H:i:s.u'): Carbon
    {
        if (is_null($datetime) || $datetime === '') {
            throw new InvalidArgumentException('datetime must not be null or empty.');
        }

        return Carbon::createFromFormat($format, $datetime, new DateTimeZone(config('app.timezone')));
    }

    /**
     * ピン番号を文字列化
     *
     * @param int $pinNumber ピン番号
     * @return string
     */
    public static function padPinNumber(int $pinNumber): string
    {
        return "pinNumber $pinNumber";
    }

    /**
     * ピン番号選択用のオプションを取得する
     *
     * @return array<int,string> ピン番号選択用のオプション
     */
    public static function pinNumberOptions(): array
    {
        return array_reduce(range(0, 127), function (array $carry, int $item) {
            $carry[$item] = self::padPinNumber($item);
            return $carry;
        }, []);
    }

    /**
     * ガントチャート表示範囲選択用のオプションを取得する。
     *
     * @return array<int,string> 表示範囲選択用のオプション
     */
    public static function ganttChartDisplayRangeOptions(): array
    {
        return array_reduce(self::ganttChartDisplayRangeMinutes(), function (array $carry, int $item) {
            $carry[$item] = __('yokakit.target_hour', ['target' => $item / 60]);
            return $carry;
        }, []);
    }

    /**
     * ガントチャートの表示範囲(分)一覧を取得する。
     *
     * @return array<int,int>
     */
    public static function ganttChartDisplayRangeMinutes(): array
    {
        return [60, 120, 180, 240, 360, 720, 1440];
    }

    /**
     * モデルが取得できていることを保証する。
     *
     * @param Model|null $model モデル
     * @return void
     * @throws ModelNotFoundException
     */
    public static function ensureModelExists(?Model $model = null): void
    {
        is_null($model) && throw new ModelNotFoundException();
    }

    /**
     * 操作結果が成功していることを保証する。
     *
     * @param Model $model モデル
     * @param bool $result false の場合に例外を投げる
     * @return void
     * @throws ModelNotFoundException
     */
    public static function ensureOperationSucceeded(Model $model, bool $result): void
    {
        $result || throw (new ModelNotFoundException())->setModel(get_class($model));
    }

    /**
     * 指定した文字列をファイル名に使用できる文字列に変換する
     *
     * @param string $filename ファイル名
     * @return string ファイル名に使用できる文字列
     */
    public static function sanitizeFileName(string $filename): string
    {
        // ファイル名に使用できない文字を正規表現で除去
        $sanitized = preg_replace('/[\\\\\\/:\*\?"<>\|\r\n\t]/', '', $filename);

        // トリムして、空白やドットで始まったり終わったりしないようにする
        $sanitized = trim((string)$sanitized, " .\t\n\r\0\x0B");

        // 不正文字のみで構成されている場合でも有効なファイル名を返す
        return $sanitized === '' ? 'download' : $sanitized;
    }
}
